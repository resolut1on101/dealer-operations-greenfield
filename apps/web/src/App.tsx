...
import { applicationEnvironment, buildVersion, databaseMigrationVersion, isSupabaseConfigured, releaseState } from './lib/environment'
import { supabase } from './lib/supabase'
import { getVisibleNavGroups } from './lib/navigation'

const visiblePackageLabel = '01U'
...
export function App() {
  ...
  const [activeItem, setActiveItem] = useState('Veri Yükleme')
  ...
  return <div className="app-shell">
    ...
    <main className={`content ${sidebarOpen ? 'sidebar-open' : 'sidebar-collapsed'}`} aria-labelledby="page-title">
      {activeItem === 'Veri Yükleme' && authenticated ? <UploadCenter role={role} /> : <AccessSurface role={role} email={email} password={password} setEmail={setEmail} setPassword={setPassword} loading={loadingAuth} authMessage={authMessage} onSignIn={signIn} />}
    </main>
  </div>
}
